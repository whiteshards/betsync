...
def setup(bot):
    ...
    def generate_board_image(self):
        ...
        try:
            # Increase board dimensions for better visibility
            board_width = 1000  # Increased from original
            board_height = 800  # Increased from original
            
            # Create a new image with a background
            img = Image.new('RGBA', (board_width, board_height), (40, 40, 60, 255))
            draw = ImageDraw.Draw(img)
            
            # Load a better font for visibility
            try:
                font = ImageFont.truetype("arial.ttf", 20)  # Larger font size
                header_font = ImageFont.truetype("arial.ttf", 24)  # Font for header
            except:
            ...
            # Draw header for multipliers
            draw.text((10, 10), f"Multipliers ({self.difficulty.capitalize()} Risk)", fill=(255, 255, 255, 255), font=header_font)
            
            # Draw multiplier values at the bottom - enhanced for visibility
            slot_width = board_width / num_slots
            
            # Draw background rectangles for multipliers
            for i in range(num_slots):
                x = i * slot_width + (slot_width / 2)
                y = board_height - 40
                
                # Determine color based on multiplier value
                mult = self.multiplier_table[i]
                
                # More distinctive color scheme based on multiplier value
                if mult >= 10:
                    bg_color = (255, 215, 0, 220)  # Gold for high multipliers
                    text_color = (0, 0, 0, 255)
                elif mult >= 5:
                    bg_color = (255, 140, 0, 220)  # Orange for medium-high multipliers
                    text_color = (0, 0, 0, 255)
                elif mult >= 2:
                    bg_color = (30, 144, 255, 220)  # Blue for medium multipliers
                    text_color = (255, 255, 255, 255)
                elif mult >= 1:
                    bg_color = (60, 179, 113, 220)  # Green for low-medium multipliers
                    text_color = (255, 255, 255, 255)
                else:
                    bg_color = (178, 34, 34, 220)  # Red for low multipliers
                    text_color = (255, 255, 255, 255)
                
                # Draw a rounded rectangle background for multiplier text
                rect_x1 = x - 25
                rect_y1 = y - 15
                rect_x2 = x + 25
                rect_y2 = y + 15
                ...
                draw.rectangle((rect_x1, rect_y1, rect_x2, rect_y2), fill=bg_color, outline=(255, 255, 255, 180), width=2)
                
                # Draw multiplier value with stronger contrast
                mult_text = f"{mult}x"
                text_width = font.getlength(mult_text) if hasattr(font, 'getlength') else font.getsize(mult_text)[0]
                
                # Position text in center of rectangle
                text_x = x - (text_width / 2)
                
                # Add a subtle shadow effect for better visibility
                draw.text((text_x+1, y-1+1), mult_text, fill=(0, 0, 0, 128), font=font)  # Shadow
                draw.text((text_x, y-1), mult_text, fill=text_color, font=font)  # Main text
            
            # Calculate grid dimensions
            peg_radius = 4
            horizontal_spacing = board_width / (num_slots + 1)
            vertical_spacing = (board_height - 100) / (actual_rows + 1)
            
            # Draw pegs
            for row in range(actual_rows):
                pegs_in_row = row + 1
                offset = (num_slots - pegs_in_row) / 2
                
                for col in range(pegs_in_row):
                    x = (offset + col + 0.5) * horizontal_spacing
                    y = 80 + (row + 1) * vertical_spacing
                    
                    # Draw peg with a highlight for better visibility
            ...
            # Draw the ball's path if there is one
            if self.ball_paths:
                path = self.ball_paths[-1]  # Get the most recent path
                ball_radius = 6
                ...
                for row, pos in enumerate(path):
                    ...
                    # Current position
                    offset_curr = (num_slots - (row + 1)) / 2
                    x_curr = (offset_curr + pos + 0.5) * horizontal_spacing
                    y_curr = 80 + (row + 1) * vertical_spacing
                    
                    # Next position
                    next_pos = path[row + 1]
                    offset_next = (num_slots - (row + 2)) / 2
                    x_next = (offset_next + next_pos + 0.5) * horizontal_spacing
                    y_next = 80 + (row + 2) * vertical_spacing
                    
                    # Draw path line
                    draw.line((x_curr, y_curr, x_next, y_next), fill=(255, 255, 0, 200), width=2)
                
                # Draw the ball at the final position
                final_row = len(path) - 1
                final_pos = path[final_row]
                offset_final = (num_slots - (final_row + 1)) / 2
                x_final = (offset_final + final_pos + 0.5) * horizontal_spacing
                y_final = 80 + (final_row + 1) * vertical_spacing
                
                # Draw ball with a glow effect
                for r in range(ball_radius + 3, ball_radius - 1, -1):
                    alpha = 100 if r > ball_radius else 255
                    color = (255, 100, 100, alpha) if r > ball_radius else (255, 50, 50, alpha)
                    draw.ellipse((x_final - r, y_final - r, x_final + r, y_final + r), fill=color)
                
                # Highlight the final multiplier
                final_slot = path[-1]
                final_mult = self.multiplier_table[final_slot]
                final_x = final_slot * slot_width + (slot_width / 2)
                
                # Draw a highlighted outline around the final multiplier
                rect_x1 = final_x - 30
                rect_y1 = board_height - 45
                rect_x2 = final_x + 30
                rect_y2 = board_height - 15
                
                # Draw pulsing highlight
                for i in range(4, 0, -1):
                    alpha = 50 + (i * 40)
                    draw.rectangle(
                        (rect_x1-i*2, rect_y1-i*2, rect_x2+i*2, rect_y2+i*2), 
                        outline=(255, 255, 100, alpha), 
                        width=2
                    )
            ...
            # If image generation fails, return a simple error image
            error_img = Image.new('RGB', (400, 300), (40, 40, 40))
            draw = ImageDraw.Draw(error_img)
            draw.text((20, 20), f"Error generating board: {e}", fill=(255, 50, 50))
            
            error_io = BytesIO()
            error_img.save(error_io, format='PNG')
            error_io.seek(0)
            return error_io
