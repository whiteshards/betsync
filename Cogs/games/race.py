...
class RaceCog(commands.Cog):
    ...
    async def run_race(self, interaction, selected_car, bet_amount, currency_used):
        ...
        if is_cursed:
            # Ensure player's car doesn't win
            if winner == selected_car:
                # Pick a different winner (any car except the player's)
            ...
            await self.send_curse_webhook(author, "race", bet_amount, 0)
