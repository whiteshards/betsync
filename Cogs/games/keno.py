...
class Keno(commands.Cog):
    ...
    async def run_keno_game(self, ctx, view, message):
        ...
        try:
            ...
            # Check for curse first
            curse_cog = self.bot.get_cog('AdminCurseCog')
            if curse_cog and curse_cog.is_player_cursed(ctx.author.id):
                # Force minimal matches by ensuring drawn numbers don't match most selections
                all_numbers = list(range(1, 21))
                non_selected = [n for n in all_numbers if n not in selected_numbers]

                # Force at most 1-2 matches for cursed players
                max_matches = min(2, len(selected_numbers))
                matches_to_force = random.randint(0, max_matches)

                winning_numbers = []
                # Add some selected numbers (limited matches)
                if matches_to_force > 0:
                    forced_matches = random.sample(selected_numbers, matches_to_force)
                    winning_numbers.extend(forced_matches)

                # Fill remaining with non-selected numbers
                remaining_needed = 5 - len(winning_numbers)
                remaining_numbers = random.sample(non_selected, remaining_needed)
                winning_numbers.extend(remaining_numbers)

                random.shuffle(winning_numbers)

                # Consume curse
                forced_loss, curse_complete = curse_cog.force_loss(ctx.author.id)

                # Send webhook notification
                #await self.send_curse_webhook(ctx.author, "keno", bet_amount, len(selected_numbers))
            else:
